...
/**
 * Normalize SQL time values (string/Date/other) to HH:MM for USSD display.
 */
function formatTimeForUSSD(timeValue) {
  if (!timeValue) return '--:--';
...
/**
 * Build a comparable Date from schedule date + departure time across DB return types.
 */
function buildScheduleDateTime(scheduleDate, departureTime) {
  if (!scheduleDate || !departureTime) return null;

  if (departureTime instanceof Date && !Number.isNaN(departureTime.getTime())) {
    const combined = new Date(scheduleDate);
    if (Number.isNaN(combined.getTime())) return departureTime;
    combined.setHours(
  ...
  const normalizedTime = formatTimeForUSSD(departureTime);
...
const handleBookingFlow = async (sessionId, phoneNumber, inputs, level) => {
  ...
  try {
    ...
    // ============================================
      ...
      // Build menu with departure and arrival times
      ...
      schedules.forEach((schedule, index) => {
        ...
        // Handle time format - might be TIME or string
        const depTime = formatTimeForUSSD(schedule.departure_time);
        const arrTime = formatTimeForUSSD(schedule.arrival_time);
        const price = parseFloat(schedule.price_per_seat).toLocaleString();
        // Use real-time calculated available seats from SQL query
        const availSeats = schedule.real_available_seats || schedule.available_seats;
        menu += `${index + 1}. ${date} (${depTime}→${arrTime}) RWF ${price} [${availSeats} seats]\n`;
      });
      return menu.trimEnd();
    }
...
2. Cancel`;
    ...
    // ============================================
      ...
      // Validate schedule hasn't departed yet
      const now = new Date();
      const scheduleDateTime = buildScheduleDateTime(
        schedule.schedule_date,
        schedule.departure_time
      );

      if (!scheduleDateTime || scheduleDateTime <= now) {
        clearSession(sessionId);
